      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-start justify-center p-6">
          ...
          <motion.div
            ...
            className="w-full max-w-sm max-h-[85vh] liquid-glass-strong rounded-[32px] p-6 flex flex-col relative overflow-hidden iridescent-border border border-white/20 shadow-2xl"
          >
            {/* Header */}
            <div className="flex items-center justify-between mb-8 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-eid-gray font-medium">Updates from the admin</p>
                </div>
              </div>
              <button
                onClick={onClose}
                className="w-10 h-10 rounded-full flex items-center justify-center bg-white/10 text-eid-dark/40 hover:text-eid-dark transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              {notifications.length === 0 ? (
                ...
                notifications.map((notif) => (
                  <motion.div
                    ...
                    className="liquid-glass-subtle rounded-2xl p-5 border border-white/10 group hover:border-eid-accent/30 transition-colors"
                  >
                    <div className="flex justify-between items-start mb-2">
                      <h3 className="font-bold text-eid-dark/90 text-lg">{notif.title}</h3>
                      <div className="flex items-center gap-1.5 text-[10px] font-bold text-eid-gray uppercase tracking-widest bg-white/5 px-2 py-1 rounded-full">
                        <Clock className="w-3 h-3" />
                        {notif.timestamp?.toDate ? formatDistanceToNow(notif.timestamp.toDate(), { addSuffix: true }) : "Just now"}
                      </div>
                    </div>
                    <p className="text-eid-gray text-sm leading-relaxed">{notif.message}</p>
                  </motion.div>
                ))
              )}
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-white/10 relative z-10">
              <p className="text-center text-[10px] font-bold text-eid-gray uppercase tracking-[0.2em] opacity-50">
